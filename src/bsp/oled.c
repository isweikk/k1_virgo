...
//Delay
void OLED_DelayMs(unsigned short ms)
{
    DelayMs(ms);
}
...
{
    OLED_WR_Byte(0xb0+y,OLED_IIC_CMD);
    OLED_WR_Byte(((x&0xf0)>>4)|0x10,OLED_IIC_CMD);
    OLED_WR_Byte((x&0x0f),OLED_IIC_CMD); 
}

//开启OLED显示    
void OLED_Display_On(void)
{
    OLED_WR_Byte(0X8D,OLED_IIC_CMD);  //SET DCDC命令
    OLED_WR_Byte(0X14,OLED_IIC_CMD);  //DCDC ON
    OLED_WR_Byte(0XAF,OLED_IIC_CMD);  //DISPLAY ON
}
...
{
    OLED_WR_Byte(0X8D,OLED_IIC_CMD);  //SET DCDC命令
    OLED_WR_Byte(0X10,OLED_IIC_CMD);  //DCDC OFF
    OLED_WR_Byte(0XAE,OLED_IIC_CMD);  //DISPLAY OFF
}
...
{  
    u8 i,n;		    
    for (i=0; i<8; i++) {  
        OLED_WR_Byte (0xb0+i,OLED_IIC_CMD);    //设置页地址（0~7）
        OLED_WR_Byte (0x00,OLED_IIC_CMD);      //设置显示位置—列低地址
        OLED_WR_Byte (0x10,OLED_IIC_CMD);      //设置显示位置—列高地址   
        for(n=0;n<128;n++) {
            OLED_WR_Byte(0,OLED_IIC_DATA); 
        }
    } //更新显示
}
...
{  
    ...
    for (i=0; i<8; i++) {  
        ...
        OLED_WR_Byte (0x00,OLED_IIC_CMD);      //设置显示位置—列低地址
        OLED_WR_Byte (0x10,OLED_IIC_CMD);      //设置显示位置—列高地址   
        for(n=0;n<128;n++)OLED_WR_Byte(1,OLED_IIC_DATA); 
    } //更新显示
}
...
{      	
    ...
    if (Char_Size ==16) {
        OLED_Set_Pos(x,y);	
        for(i=0;i<8;i++)
        OLED_WR_Byte(FontTable8x16[c*16+i],OLED_IIC_DATA);
        OLED_Set_Pos(x,y+1);
        for(i=0;i<8;i++)
        OLED_WR_Byte(FontTable8x16[c*16+i+8],OLED_IIC_DATA);
    } else {	
        OLED_Set_Pos(x,y);
        for(i=0;i<6;i++)
        OLED_WR_Byte(FontTable6x8[c][i],OLED_IIC_DATA);
...
{      			    
    ...
    for(t=0;t<16;t++)
        {
                OLED_WR_Byte(HanZiTable16x32[2*no][t],OLED_IIC_DATA);
                adder+=1;
     }	
        OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
            {	
                OLED_WR_Byte(HanZiTable16x32[2*no+1][t],OLED_IIC_DATA);
                adder+=1;
      }					
}
...
{ 	
    ...
    for (y=y0; y<y1; y++) {
        OLED_Set_Pos(x0,y);
        for (x=x0; x<x1; x++) {      
            OLED_WR_Byte(BMP[j++],OLED_IIC_DATA);	    	
        }
    }
}
...
void OLED_Num_write(unsigned char x,unsigned char y,unsigned char asc) 
{
    ...
    OLED_Set_Pos(x*6,y);
    for (i=0; i<6; i++) {
         OLED_WR_Byte(FontTable6x8[asc+16][i],OLED_IIC_DATA);         
    }
}	
void OLED_fuhao_write(unsigned char x,unsigned char y,unsigned char asc) 
{
    ...
    OLED_Set_Pos(x*6,y);
    for(i=0; i<6; i++) {
       OLED_WR_Byte(FontTable6x8[asc][i],OLED_IIC_DATA);         
    }
}			
...
void OLED_Init(void)
{
    ...
    OLED_DelayMs(800);
    OLED_WR_Byte(0xAE,OLED_IIC_CMD);//--display off
    OLED_WR_Byte(0x00,OLED_IIC_CMD);//---set low column address
    OLED_WR_Byte(0x10,OLED_IIC_CMD);//---set high column address
    OLED_WR_Byte(0x40,OLED_IIC_CMD);//--set start line address  
    OLED_WR_Byte(0xB0,OLED_IIC_CMD);//--set page address
    OLED_WR_Byte(0x81,OLED_IIC_CMD); // contract control
    OLED_WR_Byte(0xFF,OLED_IIC_CMD);//--128   
    OLED_WR_Byte(0xA1,OLED_IIC_CMD);//set segment remap 
    OLED_WR_Byte(0xA6,OLED_IIC_CMD);//--normal / reverse
    OLED_WR_Byte(0xA8,OLED_IIC_CMD);//--set multiplex ratio(1 to 64)
    OLED_WR_Byte(0x3F,OLED_IIC_CMD);//--1/32 duty
    OLED_WR_Byte(0xC8,OLED_IIC_CMD);//Com scan direction
    OLED_WR_Byte(0xD3,OLED_IIC_CMD);//-set display offset
    OLED_WR_Byte(0x00,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xD5,OLED_IIC_CMD);//set osc division
    OLED_WR_Byte(0x80,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xD8,OLED_IIC_CMD);//set area color mode off
    OLED_WR_Byte(0x05,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xD9,OLED_IIC_CMD);//Set Pre-Charge Period
    OLED_WR_Byte(0xF1,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xDA,OLED_IIC_CMD);//set com pin configuartion
    OLED_WR_Byte(0x12,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xDB,OLED_IIC_CMD);//set Vcomh
    OLED_WR_Byte(0x30,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0x8D,OLED_IIC_CMD);//set charge pump enable
    OLED_WR_Byte(0x14,OLED_IIC_CMD);//
    
    OLED_WR_Byte(0xAF,OLED_IIC_CMD);//--turn on oled panel
    OLED_Clear();
}  
